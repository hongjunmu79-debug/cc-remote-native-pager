...
.DESCRIPTION
    Runs ISCC.exe on packaging\windows\inno\cc-remote.iss with defines that pin
...
.PARAMETER StageDir
    The staged release root (contains setup.ps1, packaging/, payload/).
...
foreach ($required in @("setup.ps1", "packaging", "payload")) {
