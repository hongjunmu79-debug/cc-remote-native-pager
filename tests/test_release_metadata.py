...
def test_canonical_metadata_values_match_the_pager_distribution():
    ...
    assert metadata.distribution_version == "3.0.0-pager.15"
    ...
    assert metadata.android.version_name == "3.0.0-pager.15"
    assert metadata.android.version_code == 30024
    assert metadata.release_tag == "v3.0.0-pager.15"
...
def test_validate_release_metadata_fails_on_drifted_install_sh(tmp_path):
    ...
    fake_install.write_text(
        fake_install.read_text(encoding="utf-8").replace(
            'VERSION="${CC_REMOTE_VERSION:-3.0.0-pager.15}"',
...
def test_validate_release_metadata_cli_prints_canonical_summary():
    ...
    assert "3.0.0-pager.15" in result.stdout
