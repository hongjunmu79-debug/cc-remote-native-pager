...
METADATA = {
    ...
    "distribution_version": "3.0.0-pager.13",
    "protocol": 19,
    "repository": {"owner": "hongjunmu79-debug", "name": "cc-remote-native-pager"},
    "android": {"application_id": "dev.ccremote.lan", "version_name": "3.0.0-pager.13", "version_code": 30022},
}
...
def test_build_manifest_and_verify_round_trip(tmp_path: Path):
    ...
    assert manifest["distribution_version"] == "3.0.0-pager.13"
...
def _stage_and_build_archive(tmp_path: Path, epoch: int, name: str = "cc-remote-v3.0.0-pager.13-windows-x64.zip") -> tuple[Path, win_build.ReleaseArchive]:
...
def test_build_release_archive_is_deterministic(tmp_path: Path):
    ...
    assert first.distribution_version == "3.0.0-pager.13"
...
def _stage_and_build_portable_archive(
    tmp_path: Path,
    epoch: int,
    name: str = "cc-remote-v3.0.0-pager.13-windows-x64-portable.zip",
...
def test_portable_archive_is_deterministic_and_named_from_metadata(tmp_path: Path):
    ...
    assert default[1].path.name == "cc-remote-v3.0.0-pager.13-windows-x64-portable.zip"
...
def test_build_ps1_archive_loop_uses_split_path_leaf_under_strict_mode():
    ...
    probe = subprocess.run(
        [
            ...
            "$paths = @("
            "'C:\\out\\cc-remote-v3.0.0-pager.13-windows-x64.zip', "
            "'C:\\out\\cc-remote-v3.0.0-pager.13-windows-x64-portable.zip');"
            "$leaves = foreach ($p in $paths) { Split-Path -Leaf $p };"
            "if ($leaves[0] -ne 'cc-remote-v3.0.0-pager.13-windows-x64.zip') "
            "{ throw 'leaf0 mismatch' };"
            "if ($leaves[1] -ne "
            "'cc-remote-v3.0.0-pager.13-windows-x64-portable.zip') "
...
$installerArchivePath = Invoke-ArchiveAssembly `
    -Name 'cc-remote-v3.0.0-pager.13-windows-x64.zip' `
    -ExtraArgs @('--setup', $Setup)
$portableArchivePath = Invoke-ArchiveAssembly `
    -Name 'cc-remote-v3.0.0-pager.13-windows-x64-portable.zip' `
...
def test_build_ps1_archive_loop_captures_stdout_and_writes_real_sidecars(tmp_path: Path):
    ...
    assert zips == [
        "cc-remote-v3.0.0-pager.13-windows-x64-portable.zip",
        "cc-remote-v3.0.0-pager.13-windows-x64.zip",
    ]
